...
from routers import auth_router, crops_router, orders_router, ai_router, market_router, admin_router
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    # Only print full traceback in development to prevent info leak
    if ENVIRONMENT != "production":
        traceback.print_exc()
    else:
        import logging
        logging.getLogger(__name__).error(f"[{request_id}] Unhandled exception: {type(exc).__name__}: {exc}")
    return JSONResponse(
        status_code=500,
        content={
