...
def update_order_status(order_id: str, body: OrderStatusUpdate, user_id: str = Depends(get_current_user_id), db: Session = Depends(get_db)):
    ...
    if body.status == "cancelled":
        # Buyers can cancel only pending or confirmed orders
        if order.buyer_id == user_id:
            if current_status not in {"pending", "confirmed"}:
                raise HTTPException(
                    status_code=403,
                    detail="Buyers can only cancel pending or confirmed orders"
                )
        # Farmers can cancel only pending orders
        elif order.farmer_id == user_id:
            if current_status != "pending":
                raise HTTPException(
                    status_code=403,
                    detail="Farmers can only cancel pending orders"
                )
        else:
