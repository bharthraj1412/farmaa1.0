...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    ...
    role = "buyer"
    if hasattr(body, 'role') and body.role:
...
def google_auth(body: GoogleAuthRequest, db: Session = Depends(get_db)):
    ...
    if user is None:
        ...
        # Update profile image and name if changed — but NEVER overwrite role
        if verified_name and verified_name != "User":
            user.name = verified_name
        if profile_image:
            user.profile_image = profile_image
        # Do NOT overwrite existing user's role from client-supplied data.
        # Role changes should go through admin or a dedicated role-change flow.
        user.updated_at = datetime.now(timezone.utc)
        db.commit()
        db.refresh(user)
