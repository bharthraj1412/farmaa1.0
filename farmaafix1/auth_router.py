...
def google_auth(body: GoogleAuthRequest, db: Session = Depends(get_db)):
    ...
    verified_email = (firebase_user.get("email") or "").strip().lower()
    request_email = body.email.strip().lower()

    if verified_email and verified_email != request_email:
        raise HTTPException(
            status_code=400,
            detail="Email mismatch between token and request"
        )

    email = verified_email or request_email
    if not email:
        raise HTTPException(status_code=400, detail="No valid email found in token or request")

    firebase_uid = firebase_user.get("uid")
    google_sub = firebase_user.get("sub")

    # Find or create user — search by firebase_uid, google_id, then email
    user = None
    if firebase_uid:
...
def complete_profile(
    ...
    """Complete user profile after first Google login."""
    user = db.query(User).filter(User.id == user_id).first()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")

    # Check duplicate mobile number
    existing_with_mobile = db.query(User).filter(
        User.mobile_number == body.mobile_number,
        User.id != user_id,
    ).first()

    if existing_with_mobile:
        # ── Smart duplicate resolution ──────────────────────────────────────
        # Case 1: Orphaned legacy account (no Google/Firebase link) — safe to free the mobile
        is_orphan = not existing_with_mobile.firebase_uid and not existing_with_mobile.google_id
        # Case 2: Same email — likely a duplicate record for the same real person
        same_email = (
            existing_with_mobile.email
            and user.email
            and existing_with_mobile.email.lower() == user.email.lower()
        )
        # Case 3: Other account has mobile but profile was never completed — stale incomplete
        stale_incomplete = not existing_with_mobile.profile_completed

        if is_orphan or same_email or stale_incomplete:
            logger.warning(
                f"[Farmaa] Releasing mobile {body.mobile_number} from "
                f"account {existing_with_mobile.id} (orphan={is_orphan}, "
                f"same_email={same_email}, stale={stale_incomplete})"
            )
            existing_with_mobile.mobile_number = None
            db.commit()
        else:
            raise HTTPException(
                status_code=400,
    ...
    if body.role and body.role in _VALID_ROLES:
        user.role = body.role
    ...
    db.commit()
    db.refresh(user)
    logger.info(f"[Farmaa] Profile completed for user: {user.email} (role={user.role})")
...
def update_profile(
    ...
    if body.company_name is not None:
        user.company_name = sanitize_string(body.company_name, max_length=150) if body.company_name else None
    if body.role is not None and body.role in _VALID_ROLES:
        user.role = body.role
