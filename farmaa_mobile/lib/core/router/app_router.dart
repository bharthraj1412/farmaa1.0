  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      if (location.startsWith('/admin') && !user.isAdmin) {
        debugPrint('[Router] Non-admin user blocked from admin route');
        return AppRoutes.home;
      }

      // Farmer-only routes (crop management)
      if ((location.startsWith('/farmer/') || location == AppRoutes.myCrops) &&
          user.isBuyer &&
          !user.isAdmin) {
        debugPrint('[Router] Buyer blocked from farmer-only route: $location');
        return AppRoutes.home;
      }

      // Buyer-only routes (cart, checkout)
      if ((location == AppRoutes.cart || location.startsWith('/buyer/checkout')) &&
          user.isFarmer &&
          !user.isAdmin) {
        debugPrint('[Router] Farmer blocked from buyer-only route: $location');
        return AppRoutes.home;
      }
