    ...
    if (err.response?.statusCode == 401) {
      ...
      try {
        ...
        if (newToken != null) {
          ...
          // Clone the original request with the new token, using the main Dio instance
          final opts = err.requestOptions;
          opts.headers['Authorization'] = 'Bearer $newToken';
          opts.extra['isRetry'] = true; // Mark as retry to prevent loops
          
          // Re-execute the request using the existing Dio instance
...
class _CircuitBreakerInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (_client._isBackendDown && _client._lastFailureTime != null) {
      ...
      if (diff.inSeconds < (AppConstants.isProduction ? 10 : 15)) {
