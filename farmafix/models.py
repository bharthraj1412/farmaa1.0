...
class User(Base):
    ...
    # FIX: use lazy="select" instead of lazy="joined".
    # lazy="joined" (eager join) on every User query would load ALL crops for
    # every user fetched – extremely expensive when listing many users.
    # lazy="select" issues a separate query only when user.crops is accessed.
...
class Crop(Base):
    ...
    # "joined" is fine here: every Crop query legitimately needs its farmer.
    farmer = relationship("User", back_populates="crops", lazy="joined")
