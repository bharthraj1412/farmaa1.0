...
class _MarketPricesScreenState extends ConsumerState<MarketPricesScreen>
  ...
  // FIX: was declared `final` – district filter never actually changed.
  ...
  Future<void> _fetchPrices() async {
    ...
    try {
      final data = await CropService.instance.getMarketPrices(
        commodity: commodity,
        district: _selectedDistrict,   // FIX: now uses the mutable variable
      );
      if (mounted) setState(() => _prices = data.isNotEmpty ? data : _demoData());
    } catch (_) {
        ...
        children: [
          ...
          Padding(
            ...
            child: DropdownButtonFormField<String>(
              value: _selectedDistrict,   // FIX: bind to mutable state
              items: _tnDistricts
                  .map((d) => DropdownMenuItem(value: d, child: Text(d)))
                  .toList(),
              onChanged: (v) {
                // FIX: update mutable state and re-fetch
...
class _PriceContent extends StatelessWidget {
  ...
  LineChartData _buildChart() {
    final spots = prices.asMap().entries.map((e) {
      return FlSpot(e.key.toDouble(), (e.value['price'] as num?)?.toDouble() ?? 0);
    }).toList();
