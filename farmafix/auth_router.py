...
def google_auth(body: GoogleAuthRequest, db: Session = Depends(get_db)):
    ...
    # FIX: handle None/empty email from firebase_user explicitly
    ...
    # If firebase returned no email, trust the request email (Google always provides one)
    ...
    if user is None:
        # New user – create with minimal data (role selected at profile completion)
        user = User(
            email=email,
            firebase_uid=firebase_uid,
            google_id=google_sub,
            name=sanitize_string(body.name, max_length=100) or "User",
            role="buyer",  # default; overridden at profile completion
...
def complete_profile(
    ...
    # FIX: honour role chosen at profile completion
